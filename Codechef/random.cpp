...
#define mp make_pair
using namespace std;
/*     Author - https://www.youtube.com/channel/UCAVYawHY1vnZXjtIE4Qv3mw     */
...
vector < int > multiply(vector < int > & a, vector < int > & b, int eq = 0) {
  ...
  fill(A.begin() + a.size(), A.begin() + sz, base {
    0,
    0
  });
  fft(A, sz);
  ...
  if (eq) copy(A.begin(), A.begin() + sz, B.begin());
    ...
    fill(B.begin() + b.size(), B.begin() + sz, base {
      0,
      0
    });
    fft(B, sz);
